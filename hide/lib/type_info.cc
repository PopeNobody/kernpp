...
namespace std {
  ...
  bool type_info::__do_upcast(const __cxxabiv1::__class_type_info *__target,
			     void **__obj_ptr) const
	{
		return false;
	}
	bool type_info::operator== (const std::type_info& arg) const
	{
...
namespace __cxxabiv1 {
	...
	bool __class_type_info::__do_dyncast (
			...
			const void *src_ptr,
			__dyncast_result &__restrict result
			) const
	{
	...
	bool __class_type_info::__do_upcast (
			...
			const void *obj,
			__upcast_result &__restrict result
			) const
	{
		if (*this == *dst)
		{
	...
	bool __pointer_to_member_type_info::
		...
		{
			// This static cast is always valid, as our caller will have
			// determined that thr_type is really a __pointer_to_member_type_info.
